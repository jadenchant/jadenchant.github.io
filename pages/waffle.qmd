---
title: "Waffle"
editor_options: 
  chunk_output_type: console
execute:
  warning: false
---

```{r}
#| include: false

library(waffle)
library(magrittr)
library(ggplot2)
library(dplyr)
library(waffle)
```


```{r}
data.frame(
  parts = factor(rep(month.abb[1:3], 3), levels=month.abb[1:3]),
  vals = c(10, 20, 30, 6, 14, 40, 30, 20, 10),
  col = rep(c("navy", "black", "maroon"), 3),
  fct = c(
    rep("Thing 1", 3),
    rep("Thing 2", 3),
    rep("Thing 3", 3)
  )
) -> xdf

xdf %>%
  count(parts, wt = vals) %>%
  ggplot(
    aes(fill = parts, values = n)
  ) +
  geom_waffle(
    n_rows = 20,
    size = 0.33, 
    colour = "white",
    flip = TRUE
  ) +
  scale_fill_manual(
    name = NULL,
    values = c("#a40000", "#c68958", "#ae6056"),
    labels = c("Fruit", "Sammich", "Pizza")
  ) +
  coord_equal() +
  theme_minimal()+
  theme_enhance_waffle()
```


```{r}
#| eval: false

xdf %>%
  count(parts, wt = vals) %>%
  ggplot(
    aes(label = parts, values = n)
  ) +
  geom_pictogram(
    n_rows = 20, 
    size = 6, 
    aes(colour = parts), 
    flip = TRUE,
    family = "FontAwesome5Free-Solid"
  ) +
  scale_color_manual(
    name = NULL,
    values = c("#073f9c", "black", "#f34323"),
    labels = c("BitBucket", "GitHub", "Other")
  ) +
  scale_label_pictogram(
    name = NULL,
    values = c("bitbucket", "github", "git-alt"),
    labels = c("BitBucket", "GitHub", "Other")
  ) +
  coord_equal() +
  # theme_ipsum_rc(grid="") +
  theme_minimal()+
  theme_enhance_waffle() +
  theme(
    legend.text = element_text(hjust = 0, vjust = 1)
  )
```



```{r}
#| eval: false

waffle(c(50, 30, 15, 5), rows = 5, use_glyph = "music", glyph_size = 6)
```


```{r}
#| eval: false

library(ggplot2)
library(waffle)
library(hrbrthemes)

install_fa_fonts()

mydf <- data.frame(food_group = factor(c("Ice Cream", "Pizza", "Cake"),
    levels=c("Ice Cream", "Pizza", "Cake")), consumption = c(10, 20, 10))

# Scales and preparing for labels
scalevalues <- sprintf("%.0f%%",round(prop.table(mydf$consumption)*100, 3))
customtext <- c(
  paste("Ice cream", '=', scalevalues[1]),
  paste("Pizza", '=', scalevalues[2]),
  paste("Cake", '=', scalevalues[3])
)

ggplot(mydf, aes(label = food_group, 
                 values = consumption, 
                 color = food_group)) +
  geom_pictogram(n_rows = 10, make_proportional = TRUE) +
  scale_color_manual(
    name = NULL,
    values = c(
      `Ice Cream` = "#c68958",
      Pizza = "#ae6056",
      Cake = "#a40000"
    ),
    labels = customtext
  ) +
  scale_label_pictogram(
    name = NULL,
    values = c(
      `Ice Cream` = "ice-cream",
      Pizza = "pizza-slice",
      Cake = "birthday-cake"
    ), 
    labels = customtext) +
  coord_equal() +
  theme_ipsum_rc(grid="") +
  theme_enhance_waffle() +
  theme(legend.key.height = unit(2.25, "line")) +
  theme(legend.text = element_text(size = 10, hjust = 0, vjust = 0.75))
```








```{r}
#| eval: false

# Install and load the extrafont package
install.packages("extrafont")
library(extrafont)

# Set the source directory where the Font Awesome TTF files are located
source_font_dir <- "C:/Users/jaden/AppData/Local/R/cache/R/renv/cache/v5/R-4.3/x86_64-w64-mingw32/waffle/1.0.2/b4a5d1a3036a7e6a63439b16b3ad39fc/waffle/fonts"

# Set the destination directory where extrafont will store the fonts
destination_font_dir <- file.path(Sys.getenv("windir"), "Fonts")

# Copy Font Awesome TTF files to the system font directory
file.copy(from = list.files(source_font_dir, pattern = "*.ttf", full.names = TRUE), 
          to = destination_font_dir, 
          overwrite = TRUE)

# Register Font Awesome fonts
font_import(paths = destination_font_dir, prompt = FALSE)

# Load the Font Awesome fonts
loadfonts()

# Test the Font Awesome icons
plot.new()
text(x = 0.5, y = 0.5, labels = "\uf083", family = "FontAwesome", cex = 5, col = "darkblue")

```

